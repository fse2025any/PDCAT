...
class PDCAT:
    ...
    def run(self):
        ...
        while ts[-1] < 5000:
            ...
            while(self.constraints_check(explored_flags)):
                explored_flags = self.transProbtoflags(self.initial_pro)
            E = 0.0
            seqs.append(explored_flags)
            temp = self.get_objective_score(explored_flags, len(ts), SOURCE_PATH=self.SOURCE_PATH, GCC_PATH=self.GCC_PATH, INCLUDE_PATH=self.INCLUDE_PATH, EXEC_PARAM=self.EXEC_PARAM, LOG_FILE=self.LOG_FILE, all_flags=self.all_flags)
            res.append(temp)
